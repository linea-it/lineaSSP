...
def _build_path_from_coeff(lon_coeff, lat_coeff, t0, t1, n_elements, min_lat, max_lat, min_lon, max_lon):
    """
    ...
    try:
        ...
        # remove overflows
        idx = (longitude >= min_lon) & (longitude <= max_lon) & (latitude >= min_lat) & (latitude <= max_lat)
...
def occultation_path(
    ...
    object_radius_error = float(object_diameter_error / 2) if object_diameter_error is not None else 0
    closest_approach_error = float(closest_approach_error) if closest_approach_error is not None else 0
    error_dist_from_center = object_radius + object_radius_error + closest_approach_error
    ...
    output["central_path"] = central_path
    ...
    if object_radius > 0:
        ...
        output["body_upper_limit"] = body_upper_limit
        ...
        output["body_lower_limit"] = body_lower_limit
    ...
    if error_dist_from_center > 0:
        ...
        output["error_upper_limit"] = error_upper_limit
        ...
        output["error_lower_limit"] = error_lower_limit
